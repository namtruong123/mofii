	...
	jQuery('.draggable-form-item').click(function(e){
	    ...
	    jQuery.each(wc_custom_payment_fields_meta[field_type].fields, function(key, item){
	    	var field_name = 'field_'+ field_type + '_' + item.name.split(' ').join('-').toLowerCase() + '_' + item.type +'_'+ current_field;
				var field_value = (item.value)? item.value: '';
	    	switch(  item.type ){
	    		case 'text':
	    			html_form += '<p class="description description-wide"><label>'+ item.name +'<br/><input class="widefat code" value="'+field_value+'" type="text" name="'+	field_name	+'" /></label></p>';
	    		break;
                case 'time':
