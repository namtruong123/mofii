;( function( elementor, $, window ) {
	...
	var TemplateLibraryInsertTemplateBehavior = Marionette.Behavior.extend({
		...
		onInsertButtonClick: function onInsertButtonClick() {
			...
			$e.run('ecomus_ext/insert-template', args);
		}
	});
	...
	var TemplateLibraryTemplateView = Marionette.ItemView.extend({
		className: function className() {
			...
			if ('ecomus_ext' === source) {
				classes += ' elementor-template-library-template-' + this.model.get('type');

				if( 'woocommerce' === this.model.get('type') ) {
					classes += ' elementor-template-library-template-page';
				}
			}
	...
	var TemplateLibraryTemplateEcomusExtView = TemplateLibraryTemplateView.extend({
		template: '#tmpl-elementor-template-library-template-ecomus-ext',
		ui: function ui() {
		...
		onFavoriteCheckboxChange: function onFavoriteCheckboxChange() {
			...
			ecomus_ext_templates_lib.templates.markAsFavorite(this.model, isFavorite);

			if (!isFavorite && ecomus_ext_templates_lib.templates.getFilter('favorite')) {
	...
	var TemplateLibraryHeaderPreviewView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-preview-ecomus-ext',
	...
	var TemplateLibraryHeaderBackView = Marionette.ItemView.extend( {
		...
		onClick: function() {
			$e.routes.restoreState( 'ecomus_ext' );
		},
	} );

	var TemplateLibraryCollectionView = Marionette.CompositeView.extend({
		template: '#tmpl-elementor-template-library-templates-ecomus-ext',
		...
		getChildView: function getChildView(childModel) {
			return TemplateLibraryTemplateEcomusExtView;
		},
		initialize: function initialize() {
			this.listenTo(elementor.channels.templates, 'filter:change', this._renderChildren);
		},
		filter: function filter(childModel) {
			var filterTerms = ecomus_ext_templates_lib.templates.getFilterTerms(),
				passingFilter = true;
			jQuery.each(filterTerms, function (filterTermName) {
				var filterValue = ecomus_ext_templates_lib.templates.getFilter(filterTermName);
		...
		addSourceData: function addSourceData() {
			...
			this.$el.attr('data-template-source', isEmpty ? 'empty' : ecomus_ext_templates_lib.templates.getFilter('source'));
		},
		...
		toggleFilterClass: function toggleFilterClass() {
			this.$el.toggleClass('elementor-templates-filter-active', !!(ecomus_ext_templates_lib.templates.getFilter('text') || ecomus_ext_templates_lib.templates.getFilter('favorite')));
		},
		onRender: function onRender() {
			if ('ecomus_ext' === ecomus_ext_templates_lib.templates.getFilter('source')) {
				this.setFiltersUI();
			}
		},
		onRenderCollection: function onRenderCollection() {
			...
			if ('ecomus_ext' === ecomus_ext_templates_lib.templates.getFilter('source') && 'page' !== ecomus_ext_templates_lib.templates.getFilter('type') && 'woocommerce' !== ecomus_ext_templates_lib.templates.getFilter('type') ) {
				this.setMasonrySkin();
			}
		},
		...
		onTextFilterInput: function onTextFilterInput() {
			ecomus_ext_templates_lib.templates.setFilter('text', this.ui.textFilter.val());
		},
		onSelectFilterChange: function onSelectFilterChange(event) {
			var $select = jQuery(event.currentTarget),
				filterName = $select.data('elementor-filter');
			ecomus_ext_templates_lib.templates.setFilter(filterName, $select.val());
		},
		onMyFavoritesFilterChange: function onMyFavoritesFilterChange() {
			ecomus_ext_templates_lib.templates.setFilter('favorite', this.ui.myFavoritesFilter[0].checked);
		},
	...
	var TemplateLibraryModalLogoView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-templates-modal__header__logo-ecomus-ext',
		className: function className() {
	...
	var TemplateLibraryHeaderActionsView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-actions-ecomus-ext',
		...
		onSyncClick: function() {
			...
			ecomus_ext_templates_lib.templates.requestLibraryData({
				onUpdate: function onUpdate() {
					self.ui.sync.removeClass('eicon-animation-spin');
					$e.routes.refreshContainer('ecomus_ext');
				},
				forceUpdate: true,
				forceSync: true
			});
	...
	var TemplateLibraryHeaderMenuView = Marionette.ItemView.extend({
		...
		templateHelpers: function templateHelpers() {
			return {
				tabs: $e.components.get('ecomus_ext').getTabs()
			};
		}
	});
	...
	var TemplateLibraryLayoutView = elementorModules.common.views.modal.Layout.extend({
		...
		getLogoOptions: function() {
			return {
				title: 'Ecomus Library',
				click: function click() {
					$e.run('ecomus_ext/open', {
						toDefault: true
					});
					return false;
				}
	...
	var Component = function(ComponentModal) {
		...
		Component.prototype.show = function( args ) {
			...
			if ( args.toDefault || ! $e.routes.restoreState( 'ecomus_ext' ) ) {
		...
		Component.prototype.insertTemplate = function( args ) {
			...
			self.manager.requestTemplateContent( model.get( 'source' ), model.get( 'post_id' ), model.get( 'template_id' ), {
				data: {
	...
	var TemplateLibraryManager = function TemplateLibraryManager() {
		...
		this.requestLibraryData = function (options) {
			...
			elementorCommon.ajax.addRequest('ecomus_ext_get_library_data', ajaxOptions);
		};
		...
		this.filterTemplates = function () {
			return templatesCollection.filter(function (model) {
				if ('ecomus_ext' !== model.get('source')) {
					return false;
				}

				return 'block' === model.get('type') || 'page' === model.get('type') || 'woocommerce' === model.get('type');
			});
		};

		this.requestTemplateContent = function (source, id, template_name, ajaxOptions) {
			var options = {
				unique_id: id,
				data: {
					source: source,
	...
	ecomus_ext_templates_lib.templates = new TemplateLibraryManager();
	ecomus_ext_templates_lib.templates.init();
	...
	elementor.on('document:loaded', function() {
		...
		var $templateLibBtnStyle = $( '<style />' ),
			btnStyle = '';
			btnStyle += '.elementor-add-section-area-button.ecomus-ext-add-template-button { padding: 10px; }';
		...
		$templateLibBtn.addClass( 'elementor-add-section-area-button ecomus-ext-add-template-button' );
		$templateLibBtn.attr( 'title', 'Add Ecomus Template' );
		$templateLibBtn.html( '<img src="' + ecomus_ext_templates_lib.logoUrl + '" />' );
		...
		$previewContents.find('body').delegate('.elementor-editor-element-add', 'click', function(e) {
			setTimeout(function() {
				...
				if ( sectionInner.length > 0 ) {
					sectionInner.find( '.ecomus-ext-add-template-button' ).remove();
