jQuery(document).ready(function ($) {
	...
	// Render tabs.
	var $tabs = $( '#ecomus-size-guide-tabs' ),
		$tabsNav = $( '.ecomus-size-guide-tabs--tabs', $tabs ),
		$addTab = $( '.add-new-tab', $tabsNav ),
		templates = {
			tab: wp.template( 'ecomus-size-guide-tab' ),
			panel: wp.template( 'ecomus-size-guide-panel' )
		};

	if ( ! _.isUndefined( ecomusSizeGuideTables ) ) {
		for ( var i = 0; i < ecomusSizeGuideTables.tables.length; i++ ) {
			$addTab.before( templates.tab( {
				index: i,
				tab: ecomusSizeGuideTables.tabs[i]
			} ) );

			$tabs.append( templates.panel( {
				index: i,
				name: ecomusSizeGuideTables.names[i],
				description: ecomusSizeGuideTables.descriptions[i],
				table: ecomusSizeGuideTables.tables[i],
				information: ecomusSizeGuideTables.information[i]
			} ) );

			$tabs.find( '.ecomus-size-guide-table-editor[data-tab='+ i +']' ).find( '.ecomus-size-guide-table' ).ecomusEditTable();

			$addTab.data( 'index', i );

			if ( 0 === i ) {
				$tabsNav.children( ':eq(0)' ).addClass( 'active' );
				$tabs.find( '.ecomus-size-guide-table-editor[data-tab='+ i +']' ).addClass( 'active' );
			}
	...
	// Change active tab.
	$tabsNav.on( 'click', '.ecomus-size-guide-table-tabs--tab', function() {
		var $el = $( this );
		...
		$tabs.find( '.ecomus-size-guide-table-editor[data-tab='+ $el.data( 'tab' ) +']' ).addClass( 'active' ).siblings( '.active' ).removeClass( 'active' );
	} );

	// Edit tab title.
	$tabsNav
		.on( 'click', '.edit-button', function() {
			$( this ).closest( '.ecomus-size-guide-table-tabs--tab' ).addClass( 'editting' ).children( 'input' ).removeClass( 'hidden' );
		} )
		.on( 'click', '.confirm-button', function() {
			$( this ).closest( '.ecomus-size-guide-table-tabs--tab' ).removeClass( 'editting' ).children( 'input' ).addClass( 'hidden' );
		} )
		.on( 'input', 'input', function() {
			$( this ).siblings( '.ecomus-size-guide-table-tabs--tab-text' ).text( this.value );
		} );
	...
	$tabsNav.on( 'click', '.add-new-tab', function() {
		...
		$tabs.find( '.ecomus-size-guide-table-editor[data-tab='+ index +']' ).find( '.ecomus-size-guide-table' ).ecomusEditTable();
	...
	$tabs.on( 'click', '.delete-table', function( event ) {
		...
		var $deletedPanel = $( this ).closest( '.ecomus-size-guide-table-editor' ),
	...
	$tabs.on( 'tab_added tab_removed', function() {
		if ( $tabs.children( '.ecomus-size-guide-table-editor' ).length <= 1 ) {
...
(function ($, window, i) {
	...
	$.fn.ecomusEditTable = function (options) {

		// Settings
		var s = $.extend({
				data: [ ['']],
				tableClass: 'ecomus-table-edit',
				jsonData: false,
				headerCols: false,
				maxRows: 999,
				first_row: true,
