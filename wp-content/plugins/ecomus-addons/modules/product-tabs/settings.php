...
class Settings  {
	...
	const OPTION_NAME   = 'ecomus_product_tab';
	...
	public function __construct() {
		add_filter( 'woocommerce_get_sections_products', array( $this, 'product_tabs_section' ), 100, 2 );
		add_filter( 'woocommerce_get_settings_products', array( $this, 'product_tabs_settings' ), 100, 2 );
		...
		add_action( 'ecomus_after_product_tab_ordering', array( $this, 'clear_product_tabs_cache' ) );
		...
		add_action( 'wp_ajax_ecomus_product_tab_ordering', array( $this, 'product_tab_ordering' ) );
	...
	public function product_tabs_section( $sections ) {
		$sections['ecomus_addons_product_tabs'] = esc_html__( 'Product Tabs', 'ecomus-addons' );
	...
	public function product_tabs_settings( $settings, $section ) {
		if ( 'ecomus_addons_product_tabs' != $section ) {
			return $settings;
		}
		...
		$settings_product_tabs = array(
			...
			array(
				'name'    => esc_html__( 'Product Tabs', 'ecomus-addons' ),
				'desc'    => esc_html__( 'Enable product tabs manager', 'ecomus-addons' ),
				'id'      => self::OPTION_NAME,
				'default' => 'no',
				'type'    => 'checkbox',
				'checkboxgroup' => 'start',
	...
	public function edit_admin_columns( $columns ) {
		$columns = array_merge( $columns, array(
			'product_tab_disable' => esc_html__( 'Disabled', 'ecomus-addons' ),
			'product_tab_type' => esc_html__( 'Type', 'ecomus-addons' ),
			'product_tab_products' => esc_html__( 'Products', 'ecomus-addons' ),
			'product_tab_categories' => esc_html__( 'Categories', 'ecomus-addons' ),
		) );
	...
	public function manage_custom_columns( $column, $post_id ) {
		...
		switch ( $column ) {
			case 'product_tab_disable':
				...
				echo $tab_disable == 'yes' ? esc_html__( 'Yes', 'ecomus-addons' ) : esc_html__( 'No', 'ecomus-addons' );
			...
			case 'product_tab_type':
				...
				if ( ! is_wp_error( $types ) && $types && is_array( $types ) ) {
					...
					switch( $type_name ) {
						case 'global':
							esc_html_e('Global', 'ecomus-addons') ;
							break;
						case 'product':
							esc_html_e('Product', 'ecomus-addons') ;
							break;
						case 'custom':
							esc_html_e('Custom', 'ecomus-addons') ;
							break;
						default:
							esc_html_e('Default', 'ecomus-addons') ;
							break;
					}
				} else {
					esc_html_e( 'No Type', 'ecomus-addons' );
				}
			...
			case 'product_tab_products':
				...
				if (  $product_ids ) {
					...
					$links[] = esc_html__( 'No Product', 'ecomus-addons' );
				}
			...
			case 'product_tab_categories':
				...
				if ( ! is_wp_error( $cats ) && $cats && is_array( $cats ) ) {
					...
					$links[] = esc_html__( 'No Category', 'ecomus-addons' );
				}
	...
	public function meta_boxes( $post ) {
		add_meta_box( 'ecomus-product-tabs', esc_html__( 'Tabs Settings', 'ecomus-addons' ), array( $this, 'tabs_meta_box' ), self::POST_TYPE, 'advanced', 'high' );
	}
	...
	public function tabs_meta_box( $post ) {
		?>
		<div id="ecomus-product-tabs-settings" class="ecomus-product-tabs-settings">
			<p class="form-field">
				<label><?php esc_html_e('Tab Type', 'ecomus-addons'); ?></label>
				<span class="ecomus-product-tabs--type">
					<?php
					$terms = get_the_terms( $post->ID, self::TAXONOMY_TAB_TYPE );
					$tab_type = ! is_wp_error( $terms ) && $terms ? $terms[0]->name : 'global';
					?>
					<input type="radio" id="ecomus-product-tab-global" <?php echo $tab_type == 'global' ? 'checked' : ''; ?> class="ecomus-product-tab--input" checked name="_product_tab_type" value="global">
					<label for="ecomus-product-tab-global"><?php esc_html_e('Global', 'ecomus-addons'); ?></label>
					<input type="radio" id="ecomus-product-tab-product" <?php echo $tab_type == 'product' ? 'checked' : ''; ?> class="ecomus-product-tab--input" name="_product_tab_type" value="product">
					<label for="ecomus-product-tab-product"><?php esc_html_e('Product', 'ecomus-addons'); ?></label>
					<input type="radio" id="ecomus-product-tab-custom" <?php echo $tab_type == 'custom' ? 'checked' : ''; ?> class="ecomus-product-tab--input" name="_product_tab_type" value="custom">
					<label for="ecomus-product-tab-custom"><?php esc_html_e('Custom', 'ecomus-addons'); ?></label>
				</span>

			</p>
			<p class="form-field ecomus-product-tabs--product">
				<label for="product-tab-id"><?php esc_html_e( 'Products', 'ecomus-addons' ); ?></label>
				<select class="wc-product-search" multiple="multiple" style="width: 50%;" id="product-tab-id" name="_product_tab_product_ids[]" data-sortable="true" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'ecomus-addons' ); ?>" data-action="woocommerce_json_search_products">
					<?php
			...
			<p class="form-field ecomus-product-tabs--categories">
				<label for="product_tab_categories"><?php esc_html_e( 'Categories', 'ecomus-addons' ); ?></label>
				<select class="wc-category-search" multiple="multiple" style="width: 50%;" id="product_tab_categories" name="_product_tab_cat_slugs[]" data-sortable="true" data-placeholder="<?php esc_attr_e( 'Search for a category&hellip;', 'ecomus-addons' ); ?>" data-action="woocommerce_json_search_categories">
					<?php
	...
	public function create_post_type() {
		$posts = array(
			'description' 				=>	esc_html__('Description', 'ecomus-addons'),
			'additional_information' 	=>	esc_html__('Additional Information', 'ecomus-addons'),
			'reviews' 					=>	esc_html__('Reviews', 'ecomus-addons'),
		);
	...
	public static function product_tab_ordering() {
		...
		$menu_orders = wp_list_pluck( $wpdb->get_results( "SELECT ID, menu_order FROM {$wpdb->posts} WHERE post_type = 'ecomus_product_tab' ORDER BY menu_order DESC" ), 'menu_order', 'ID' );
		...
		foreach ( $menu_orders as $id => $menu_order ) {
			...
			*/
			do_action( 'ecomus_after_single_product_tab_ordering', $id, $index );
		}
		...
		do_action( 'ecomus_after_product_tab_ordering', $sorting_id, $menu_orders );
	...
	function product_tab_redirect() {
		global $pagenow;
		if($pagenow == 'post.php' && isset($_GET['post']) && get_post_type( $_GET['post'] ) == 'ecomus_product_tab'){
			$terms = get_the_terms( $_GET['post'], self::TAXONOMY_TAB_TYPE );
			$tab_type = ! is_wp_error( $terms ) && $terms ? $terms[0]->name : '';
			if( $tab_type == 'default' ) {
				wp_redirect(admin_url('/edit.php?post_type=ecomus_product_tab' ));
				exit;
			}
	...
	public function clear_product_tabs_cache() {
		delete_transient( 'ecomus_wc_product_tabs' );
	}
	...
	public function admin_scripts( $hook ) {
		...
		if ( in_array( $hook, array('edit.php', 'post-new.php', 'post.php' ) ) && self::POST_TYPE == $screen->post_type ) {
			wp_enqueue_style( 'ecomus-product-tabs', ECOMUS_ADDONS_URL . 'modules/product-tabs/assets/product-tabs-admin.css' );
			wp_enqueue_script( 'ecomus-product-tabs', ECOMUS_ADDONS_URL . 'modules/product-tabs/assets/product-tabs-admin.js', array( 'jquery', 'jquery-ui-sortable' ),'1.0', true );
