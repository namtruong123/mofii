...
class Settings  {
	...
	const OPTION_NAME   = 'ecomus_popup';
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_ecomus_save_popup_enable', array( $this, 'save_popup_enable' ) );
		...
		add_action( 'wp_ajax_ecomus_popup_ordering', array( $this, 'popup_ordering' ) );
	...
	public function edit_admin_columns( $columns ) {
		$columns = array_merge( $columns, array(
			'popup_include_page' 	=> esc_html__( 'Include Pages', 'ecomus-addons' ),
			'popup_exclude_page' 	=> esc_html__( 'Exclude Pages', 'ecomus-addons' ),
			'popup_enabled' => esc_html__( 'Enabled', 'ecomus-addons' ),
		) );
	...
	public function manage_custom_columns( $column, $post_id ) {
		...
		switch ( $column ) {
			case 'popup_enabled':
				...
				echo '<div class="ecomus-popup__toggle-button">';
				echo sprintf(
					'<input type="checkbox" id="popup_enabled_%1$s" class="ecomus-popup__enabled" %2$s data-nonce="ecomus_save_enabled_state_%1$s" data-popup-id="%1$s">',
					esc_attr( $post_id ),
					$checked
				);
				echo '<label for="ecomus_popup_enable'. esc_attr( $post_id ) .'" aria-label="Switch to enable popup"></label>';
				echo '</div>';
				break;
			...
			case 'popup_include_page':
				...
				if( empty( $page_ids ) ) {
					echo esc_html__( 'All', 'ecomus-addons' );
				} else {
	...
	public static function popup_ordering() {
		...
		$menu_orders = wp_list_pluck( $wpdb->get_results( "SELECT ID, menu_order FROM {$wpdb->posts} WHERE post_type = 'ecomus_popup' ORDER BY menu_order DESC" ), 'menu_order', 'ID' );
		...
		foreach ( $menu_orders as $id => $menu_order ) {
			...
			*/
			do_action( 'ecomus_after_single_popup_ordering', $id, $index );
		}
		...
		do_action( 'ecomus_after_popup_ordering', $sorting_id, $menu_orders );
	...
	public function admin_scripts( $hook ) {
		...
		if ( in_array( $hook, array('edit.php', 'post-new.php', 'post.php' ) ) && self::POST_TYPE == $screen->post_type ) {
			wp_enqueue_style( 'ecomus-popup', ECOMUS_ADDONS_URL . 'modules/popup/assets/css/admin.css' );
			wp_enqueue_script( 'ecomus-popup', ECOMUS_ADDONS_URL . 'modules/popup/assets/js/admin.js', array( 'jquery', 'jquery-ui-sortable' ),'1.0', true );
