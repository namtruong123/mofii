...
class Widget extends \WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'ecomus-products-filter',
			esc_html__( 'Ecomus - Products Filter', 'ecomus-addons' ),
			array(
				'classname'                   => 'products-filter-widget woocommerce',
				'description'                 => esc_html__( 'WooCommerce products filter.', 'ecomus-addons' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( $title = apply_filters( 'widget_title', $instance['title'], $instance, $this->id_base ) ) {
			echo $args['before_title'] . \Ecomus\Addons\Helper::get_svg( 'filter' ) . esc_html( $title ) . $args['after_title'];
		}
		...
		if ( ! empty( $instance['filter'] ) ) {
			...
			echo '<button type="reset" value="' . esc_attr__( 'Reset Filter', 'ecomus-addons' ) . '" class="button em-button-outline-dark alt reset-button ecomus-button--ghost ecomus-button--color-black">' . esc_html__( 'Clear', 'ecomus-addons' ) . '</button>';
			echo '</div>';
		...
		if ( ! empty( $filter['show_view_more'] ) && empty( $filter['scrollable'] ) && in_array( $filter['display'], array( 'list', 'checkboxes' ) ) ) {
			$view_more = sprintf(
				'<div class="ecomus-widget-product-cats-btn">
					<span class="em-button-subtle show-more">%s</span>
					<span class="em-button-subtle show-less">%s</span>
		...
		switch ( $filter['source'] ) {
			...
			case 'products_group':
				$filter_groups = apply_filters( 'ecomus_products_filter_groups', array(
					'best_sellers' => esc_attr__( 'Best Sellers', 'ecomus-addons' ),
	...
	public function form( $instance ) {
		...
		<div class="ecomus-products-filter-form__sub-nav">
			<button type="button" data-section="filters" class="button-link active"><?php esc_html_e( 'Filters', 'ecomus-addons' ); ?></button> |
			<!-- <button type="button" data-section="design" class="button-link"><?php esc_html_e( 'Design', 'ecomus-addons' ); ?></button> | -->
			<button type="button" data-section="options" class="button-link"><?php esc_html_e( 'Options', 'ecomus-addons' ); ?></button>
		</div>

		<p><hr/></p>

		<div class="ecomus-products-filter-form__section active" data-section="filters">
			<p class="ecomus-products-filter-form__message <?php echo ! empty( $instance['filter'] ) ? 'hidden' : '' ?>"><?php esc_html_e( 'There is no filter yet.', 'ecomus-addons' ) ?></p>

			<div class="ecomus-products-filter-form__filter-fields">
				<?php $this->filter_setting_fields( $instance['filter'] ); ?>
			</div>

			<p class="ecomus-products-filter-form__section-actions">
				<button type="button" class="ecomus-products-filter-form__add-new button-link" data-name="<?php echo esc_attr( $this->get_field_name( 'filter' ) ); ?>" data-count="<?php echo count( $instance['filter'] ) ?>">+ <?php esc_html_e( 'Add a new filter', 'ecomus-addons' ) ?></button>
			</p>
		</div>

		<div class="ecomus-products-filter-form__section" data-section="design">
		</div>

		<div class="ecomus-products-filter-form__section" data-section="options">
			<?php
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'ajax',
				'label' => esc_html__( 'Use ajax for filtering', 'ecomus-addons' ),
				'value' => $instance['ajax'],
			) );
			...
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'instant',
				'label' => esc_html__( 'Filtering products instantly (no buttons required)', 'ecomus-addons' ),
			...
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'change_url',
				'label' => esc_html__( 'Update URL', 'ecomus-addons' ),
	...
	protected function filter_setting_fields( $fields = array(), $context = 'display' ) {
		...
		foreach ( $filter_fields as $index => $field ) :
			...
			?>
			<div class="ecomus-products-filter-form__filter">
				<div class="ecomus-products-filter-form__filter-top">
					<div class="ecomus-products-filter-form__filter-actions">
						<button type="button" class="ecomus-products-filter-form__remove-filter button-link button-link-delete">
							<span class="screen-reader-text"><?php esc_html_e( 'Remove filter', 'ecomus-addons' ) ?></span>
							<span class="dashicons dashicons-no-alt"></span>
						</button>
					</div>

					<button type="button" class="ecomus-products-filter-form__filter-toggle">
						<span class="ecomus-products-filter-form__filter-toggle-indicator" aria-hidden="true"></span>
					</button>

					<div class="ecomus-products-filter-form__filter-title"><?php echo $title; ?></div>
				</div>
				<div class="ecomus-products-filter-form__filter-options">
					<?php
					foreach ( $filter_settings as $name => $options ) {
						...
						$options['class'] = 'ecomus-products-filter-form__filter-option';
	...
	protected function get_filter_fields_settings() {
		...
		$this->filter_settings = array(
			...
			'ranges' => array(
				...
				'desc' => __( 'Each range on a line, separate by the <code>-</code> symbol. Do not include the currency symbol.', 'ecomus-addons' ),
				'condition' => array(
					'display' => 'ranges',
					'source'  => 'price',
				),
			...
			'searchable' => array(
				'type' => 'checkbox',
				'label' => __( 'Show the search box', 'ecomus-addons' ),
				'condition' => array(
			...
			'cats_numbers'                   => array(
				'type'  => 'number',
				'label' => esc_html__( 'Categories per view', 'ecomus-addons' ),
				'condition' => array(
					'source'  => array( 'product_cat' ),
					'display' => array( 'list', 'checkboxes' ),
					'show_view_more!' => '',
			...
			'disable_onsale'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Disable on sale', 'ecomus-addons' ),
				'condition' => array(
			...
			'disable_in_stock'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Disable in stock', 'ecomus-addons' ),
				'condition' => array(
					'source'  => array( 'product_status' ),
				),
			),
			'disable_out_of_stock'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Disable out of stock', 'ecomus-addons' ),
				'condition' => array(
					'source'  => array( 'product_status' ),
				),
			),
	...
	public function admin_scripts( $hook ) {
		...
		wp_enqueue_style( 'ecomus-products-filter-admin', ECOMUS_ADDONS_URL . 'modules/products-filter/assets/css/products-filter-admin.css', array(), '20210311' );
		wp_enqueue_script( 'ecomus-products-filter-admin', ECOMUS_ADDONS_URL . 'modules/products-filter/assets/js/products-filter-admin.js', array( 'wp-util' ), '20210311', true );

		wp_localize_script(
			'ecomus-products-filter-admin', 'ecomus_products_filter_params', array(
				'sources'    => $this->get_filter_source_options(),
	...
	public function scripts() {
		...
		wp_enqueue_script( 'ecomus-products-filter', ECOMUS_ADDONS_URL . 'modules/products-filter/assets/js/products-filter.js', array(
			'jquery',
			'wp-util',
			'select2',
	...
	public function filter_setting_fields_template() {
        ...
        <script type="text/template" id="tmpl-ecomus-products-filter">
