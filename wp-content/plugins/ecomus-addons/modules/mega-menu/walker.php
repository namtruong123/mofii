...
class Walker extends \Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ( ! $depth && $this->in_mega ) {
			...
			if ( 'container-fluid' != self::$mega_data['width'] ) {
				$classes[] = self::$mega_data['alignment'] ? 'mega-menu-alignment--'. self::$mega_data['alignment'] : 'mega-menu-alignment--center';
			}
		}
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $item_mega['mega'] && ! $depth ) {
			...
			if ( $data_grid ) {
				...
				if ( self::$mega_data['alignment'] != 'center' && self::$mega_data['width'] == 'custom' ) {
					$classes[] = 'menu-item-pos-alignment--' . self::$mega_data['alignment'];
					$classes[] = 'menu-item-pos-static';
				}
			}
		}
	...
	public function display_grid( $item, $args = null ) {
		...
		$alignment = self::$mega_data['alignment'] ? ' mega-menu-alignment--'. self::$mega_data['alignment'] : ' mega-menu-alignment--center';

		$output  = '<ul class="sub-menu mega-menu mega-menu--grid'. $alignment .'" '. $this->get_mega_menu_css() .'>';
		...
		if ( $widget['is_widget'] && 'false' !== $widget['is_widget'] ) {
			...
			if ( $widget['object'] ) {
				...
				if ( isset( $widget['spacing'] ) && ! empty( $widget['spacing'] ) && $widget['object'] == 'button' ) {
					$data = 'style=--em-menu-button-item-spacing:' . $widget['spacing'] . 'px';
				}
