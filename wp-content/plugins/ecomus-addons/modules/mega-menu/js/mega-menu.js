/* global wp, wpNavMenu, ecomusAddonsMegaMenuConfig */
var kmm = kmm || {};

( function( $, _, wp ) {
	hasModified = false;
	var config = window.ecomusAddonsMegaMenuConfig || {};

	if ( ! config ) {
		return false;
	}

	config.namespace = 'ecomus_addons_megamenu';
	...
	MegaMenu.prototype.onClosed = function( event ) {
		...
		$( '.submit-add-to-menu', '#ecomus_addons_mega_items' ).prop( 'disabled', true );
