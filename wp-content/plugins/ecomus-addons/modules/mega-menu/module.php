...
namespace Ecomus\Addons\Modules\Mega_Menu;
...
class Module {
	...
	protected function init() {
		...
		add_action( 'wp_ajax_ecomus_addons_megamenu_save', array( $this, 'save_menu_item_data' ) );
		add_action( 'wp_ajax_ecomus_addons_megamenu_get_grid_items', array( $this, 'get_menu_items' ) );
		add_action( 'wp_ajax_ecomus_addons_megamenu_update_grid', array( $this, 'save_menu_grid_data' ) );
		add_action( 'wp_ajax_ecomus_addons_megamenu_update_tab', array( $this, 'save_menu_tab_data' ) );
	...
	public function add_settings_link( $item_id ) {
		...
		<fieldset class="field-mega-options hide-if-no-js description-wide">
			<span class="field-move-visual-label" aria-hidden="true"><?php echo esc_html( _x( 'Options', 'Mega menu options', 'ecomus-addons' ) ) ?></span>
			...
			<button type="button" class="item-config-mega button-link"><?php esc_html_e( 'Mega Menu', 'ecomus-addons' ) ?></button>
			<button type="button" class="item-config-icon button-link"><?php esc_html_e( 'Icon', 'ecomus-addons' ) ?></button>
		</fieldset>
		<?php
	}
	...
	public function setup_nav_menu_item( $menu_item ) {
		...
		if ( 0 === strpos( $menu_item->type, 'megamenu_item__' ) ) {
			...
			$menu_item->type_label = __( 'Mega Item', 'ecomus-addons' );
		}
	...
	public function scripts( $hook ) {
		...
		wp_register_style( 'ecomus-addons-mega-menu-admin', $assets_url . '/css/mega-menu.css', array(
			'media-views',
			'wp-color-picker',
		), '1.0' );
		wp_enqueue_style( 'ecomus-addons-mega-menu-admin' );

		wp_register_script( 'ecomus-addons-mega-menu-admin', $assets_url . '/js/mega-menu.js', array(
			'jquery',
			'jquery-ui-resizable',
			'wp-util',
			'wp-color-picker',
		...
		wp_enqueue_media();
		wp_enqueue_script( 'ecomus-addons-mega-menu-admin' );

		wp_enqueue_script( 'ecomus-addons-menu-item', $assets_url . '/js/menu-item.js', array( 'jquery' ),'1.0', true );

		wp_localize_script( 'ecomus-addons-mega-menu-admin', 'ecomusAddonsMegaMenuConfig', array(
			'templates' => $this->get_modal_templates(),
			'l10n' => array(
				'close_confirm'       => esc_html__( 'Your changes are not saved. Do you want to leave?', 'ecomus-addons' ),
				'enable_mega_message' => esc_html__( 'You need to enable the mega menu first', 'ecomus-addons' ),
				'width_auto'          => esc_html__( 'Auto', 'ecomus-addons' ),
			),
		) );
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'mega-menu-script', $assets_url . '/js/scripts.js', array( 'jquery' ), '1.0.0', true );
	...
	public function meta_boxes() {
		add_meta_box( 'ecomus_addons_mega_menu_items', __( 'Mega Menu Items', 'ecomus-addons' ), array( $this, 'nav_menu_items' ), 'nav-menus', 'side', 'low' );
	}
	...
	public function nav_menu_items() {
		...
		<div id="posttype-megamenu_item" class="posttypediv">
			<div id="tabs-panel-posttype-megamenu_item" class="tabs-panel tabs-panel-active" role="region" aria-label="<?php esc_attr_e( 'Mega Menu Item List', 'ecomus-addons' ) ?>" tabindex="0">
			...
			<p class="button-controls wp-clearfix">
				<span class="add-to-menu">
					<button type="submit" class="button submit-add-to-menu right" value="<?php esc_attr_e( 'Add to Menu', 'ecomus-addons' ); ?>" name="add-post-type-menu-item" id="submit-posttype-megamenu_item" disabled><?php esc_html_e( 'Add to Menu', 'ecomus-addons' ); ?></button>
	...
	public static function get_item_widgets() {
		return apply_filters( 'ecomus_addons_mega_menu_items', array(
			'image' 			=> '\Ecomus\Addons\Modules\Mega_Menu\Widgets\Image',
			'banner' 			=> '\Ecomus\Addons\Modules\Mega_Menu\Widgets\Banner',
			'html'  			=> '\Ecomus\Addons\Modules\Mega_Menu\Widgets\HTML',
			'heading'  			=> '\Ecomus\Addons\Modules\Mega_Menu\Widgets\Heading',
			'button'  			=> '\Ecomus\Addons\Modules\Mega_Menu\Widgets\Button',
			'custom-link'  		=> '\Ecomus\Addons\Modules\Mega_Menu\Widgets\Custom_Link',
			'products-carousel' => '\Ecomus\Addons\Modules\Mega_Menu\Widgets\Products_Carousel',
			'taxonomy-list' 	=> '\Ecomus\Addons\Modules\Mega_Menu\Widgets\Taxonomy_List',
		) );
	}
		...
		<div id="megamenu-modal" class="megamenu-modal">
			<div class="megamenu-modal__modal media-modal wp-core-ui" tabindex="0">
				<button type="button" class="media-modal-close megamenu-modal__close">
					<span class="media-modal-icon"><span class="screen-reader-text"><?php esc_html_e( 'Close', 'ecomus-addons' ) ?></span></span>
				</button>
				<div class="media-modal-content">
					...
					<div class="media-frame-toolbar">
						<div class="megamenu-modal__toolbar media-toolbar">
							<button type="button" class="megamenu-modal__save button media-button button-primary button-large" disabled><?php esc_html_e( 'Save Changes', 'ecomus-addons' ) ?></button>
							<button type="button" class="megamenu-modal__cancel button media-button button-secondary button-large"><?php esc_html_e( 'Cancel', 'ecomus-addons' ) ?></button>
							<span class="spinner"></span>
						</div>
					</div>
				</div>
			...
			<div class="megamenu-modal__modal wp-core-ui" tabindex="0">
				...
				<div class="megamenu-modal__frame">
					...
					<div class="megamenu-modal__toolbar">
						<button type="button" class="megamenu-modal__save button button-primary button-large" disabled><?php esc_html_e( 'Save Changes', 'ecomus-addons' ) ?></button>
						<button type="button" class="megamenu-modal__cancel button button-secondary button-large"><?php esc_html_e( 'Cancel', 'ecomus-addons' ) ?></button>
						<span class="spinner"></span>
					</div>
				</div>
			</div>
	...
	public function save_menu_item_data() {
		...
		if ( ! empty( $data['icon_svg'] ) ) {
			$data['icon_svg'] = \Ecomus\Addons\Helper::sanitize_svg( $data['icon_svg'] );
		}
	...
	public function get_menu_items() {
		...
		while ( $i < count( $items ) ) {
			...
			if ( ! $menu_obj->is_widget ) {
				...
				$menu_obj->type_label    = __( 'Mega Item', 'ecomus-addons' );
	...
	public static function default_settings() {
		return apply_filters(
			'ecomus_addons_mega_menu_default',
			array(
				'mega'         	=> false,
				'mega_mode'    	=> 'default',
	...
	public function get_menu_icon( $item ) {
		...
		switch ( $data['icon_type'] ) {
			case 'theme':
				if ( ! empty( $data['icon'] ) ) {
					$icon = \Ecomus\Addons\Helper::get_svg( $data['icon'], 'social', array( 'class' => 'menu-item-icon em-socials-button em-socials--' . $data['icon'] ) );
				}
				break;
			...
			case 'image':
				if ( ! empty( $data['icon_image'] ) ) {
					...
					$icon = '<span class="menu-item-icon menu-icon-item--image em-ratio">'. $image_html .'</span>';
				}
				break;
