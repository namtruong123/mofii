    ...
    function yozi_get_category_childs( $categories, $id_parent, $level, &$dropdown ) {
        foreach ( $categories as $key => $category ) {
            if ( $category->category_parent == $id_parent ) {
                $dropdown = array_merge( $dropdown, array( str_repeat( "- ", $level ) . $category->name . '('.$category->term_id.')' => $category->slug ) );
...
function yozi_woocommerce_relation_product_options()
{
    ...
    if(!empty($customer_also_viewed))
    {
        foreach($customer_also_viewed as $viewed)
        {
            ...
            $count_option_value = 0;
            if ( is_array($option_value) ) {
                $count_option_value = count($option_value);
            }
            $option_value = $count_option_value > 1 ? implode(',', $option_value) : $post->ID;
...
if ( !function_exists('yozi_woocommerce_enqueue_styles') ) {
    function yozi_woocommerce_enqueue_styles() {
        ...
        wp_register_script( 'yozi-woocommerce', get_template_directory_uri() . '/js/woocommerce.js', array( 'jquery', 'jquery-unveil', 'slick', 'photoswipe', 'zoom' ), '20150330', true );
...
function yozi_display_products_block_by_category($blocks, $parent_slug) {
    ...
    foreach ($blocks as $key => $value) {
        ...
        if ( (is_shop() && yozi_get_config( 'show_'.$key.'_in_shop', true ) ) || (is_array($categories) && is_object($term) && in_array($term->slug, $categories)) ) {
...
add_filter( 'woosc_button_position_archive_default', 'yozi_woosc_button_position_archive_default' );
function yozi_woosc_button_position_archive_default($return) {
    return '';
}

add_filter( 'woosw_button_position_archive_default', 'yozi_woosw_button_position_archive_default' );
function yozi_woosw_button_position_archive_default($return) {
    return '';
}